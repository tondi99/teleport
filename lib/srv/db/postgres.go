...
func (p *postgresProxy) handleConnection(ctx context.Context, conn net.Conn) (err error) {
	...
	startupMessage, tlsConn, err := p.handleStartup(ctx, conn)
	if err != nil {
...
func (p *postgresProxy) handleStartup(ctx context.Context, conn net.Conn) (*pgproto3.StartupMessage, *tls.Conn, error) {
...
func (e *postgresEngine) handleConnection(ctx context.Context, sessionCtx *sessionContext, conn net.Conn) error {
	...
	// i.e. this is where psql prompt appears on the other side.
...
func (e *postgresEngine) getConnectConfig(ctx context.Context, sessionCtx *sessionContext) (*pgconn.Config, error) {
	...
	config, err := pgconn.ParseConfig(fmt.Sprintf("postgres://%s@%s/?database=%s",
		sessionCtx.dbUser, sessionCtx.db.Endpoint, sessionCtx.dbName))
	if err != nil {
