...
type LocalKeyStore interface {
	...
	GetKey(proxy, username, clusterName string) (*Key, error)

	// DeleteKey removes a specific session key from a proxy.
	DeleteKey(proxyHost, username, clusterName string) error
...
func (fs *FSLocalKeyStore) DeleteKey(host, username, clusterName string) error {
	...
	if clusterName != "" {
		kubeCertsDir := filepath.Join(dirPath, username+kubeDirSuffix, clusterName)
...
func (fs *FSLocalKeyStore) GetKey(proxyHost, username, clusterName string) (*Key, error) {
	...
	// TODO(awly): unit test this.
	if clusterName != "" {
		kubeDir := filepath.Join(dirPath, username+kubeDirSuffix, clusterName)
