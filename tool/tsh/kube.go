...
func (c *kubeCredentialsCommand) run(cf *CLIConf) error {
	...
	k, err := tc.LocalAgent().GetKey(c.teleportCluster)
...
func (c *kubeClustersCommand) run(cf *CLIConf) error {
	...
	if kc, err := kubeconfig.Load(""); err != nil {
		...
		selectedCluster = strings.TrimPrefix(kc.CurrentContext, currentTeleportCluster+"-")
	}
...
func (c *kubeLoginCommand) run(cf *CLIConf) error {
	...
	kubeContext := fmt.Sprintf("%s-%s", profile.Cluster, c.kubeCluster)
